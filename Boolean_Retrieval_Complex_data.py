...
def Posting_list_merge(query_words, inverted_list):
    ...
    for word in query_words:
        if word not in ['and', 'or', 'not']:
            try:
                document_ids = inverted_list.get_value(index=word, col='document-id')
            except KeyError:
...
#    step1: read files with tokenization
    token_list = []
    for i in range(document_num):
        ...
        document_sizes[i] = len(token)
        if len(token) == 0:
            print('the size is 0, the file id is:', i+1)
        if tokens is None:
            tokens = token
        else:
            if len(token) != 0:
                tokens = np.concatenate([tokens, token], axis=0)
        if i % 500 == 0:
    ...
    for i in range(len(token_list)):
        if tokens is None:
            ...
            tokens = np.concatenate([tokens, token_list[i]], axis=0)
    import gc
    print('the size of token_list:', sys.getsizeof(token_list))
    del token_list
    del token
    gc.collect()
    ...
    for i in range(len(tokens)):
        tokens[i, 0] = snowstemmer.stem(tokens[i, 0])

    # step3: sorting
    tokens_snow = sorting_tokens(tokens=tokens)
    del tokens
    gc.collect()

    # # step4: transforming into posting
    df_snow = pd.DataFrame(data=tokens_snow, columns=['word', 'document-id'])
    inverted_list_snow = df_snow.groupby("word").apply(lambda x: pd.Series(dict([[vv, x[vv].tolist()] for vv in x if vv not in ['word']])))
    del tokens_snow
    del df_snow
    gc.collect()
    print('the time on creating index:', time.time() - start1)
    print('The memory size of storing posting list with SnowStemmer', sys.getsizeof(inverted_list_snow))
    start2 = time.time()
    ...
    document_list = Posting_list_merge(query_words=query_words, inverted_list=inverted_list_snow)
