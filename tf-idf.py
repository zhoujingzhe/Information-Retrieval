...
if __name__ == "__main__":
    ...
    inverted_list = dictionary

    # calculate tf-idf
    tf_matrix = np.zeros((len(inverted_list.keys()), document_num))
    idf_matrix = np.zeros((len(inverted_list.keys())))
    d_f = list(inverted_list.values())
    index = 0
    for y in d_f:
    ...
    word_indexes = list(inverted_list.keys())
    qword_list = []
    for i in range(len(query_words)):
        try:
            qword_list.append(word_indexes.index(query_words[i]))
        except ValueError:
            print('the word:'+ query_words[i] +'does not exist in database!')
            word = getsimilarword(word=query_words[i], dictionary=list(inverted_list.keys()))
            print('Thus, replaced by the word', word)
            qword_list.append(word_indexes.index(word))

    counter = collections.Counter(qword_list)
    q_tf = np.zeros((len(inverted_list.keys())))
    q_tf[np.array(list(map(int, map(float, np.unique(qword_list)))))] = np.array(list(counter.values()))
    q_tf = np.log(1 + np.divide(q_tf, len(query_words)))
    q_tf = np.reshape(a=q_tf, newshape=(-1, 1))
    q_tfidf = q_tf * idf
    print(get_cosine_score(q_tfidf=q_tfidf, tf_idf=tf_idf))
    print('the time spent on retrieval is:', time.time()-start2)
